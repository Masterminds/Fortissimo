 ...
 * @code
 * <?php
 * $register->cache('\Fortissimo\Cache\Memcache', 'memcache')
 *   ->using('servers', array('example.com:11211', 'example.com:11212'))
 *   ->using('persistent', FALSE)
 *   ->using('compress', TRUE)
 *   ->using('isDefault', TRUE)
 * ;
 * ?>
 * @endcode
 *
...
class Memcache extends Base {
  ...
  public function init() {
    ...
    if (empty($this->params['server'])) {
       throw \Fortissimo\InterruptException('No memcache server was specified, but init was attempted.');
    }
